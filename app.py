"""ReverseOrbitLLM — Browser-based model liberation with chat playground.

Deploy on HuggingFace Spaces (ZeroGPU — users bring their own GPU quota)
or run locally:
    pip install -e ".[spaces]"
    reverseorbitllm ui              # beautiful launcher with GPU detection
...
if "USER" not in os.environ:
    os.environ["USER"] = "reverseorbitllm"
...
from reverseorbitllm import device as dev
...
_SESSION_META_FILE = "reverseorbitllm_session.json"
...
def _recover_sessions_from_disk() -> None:
    ...
    found_any = False
    for pattern in ("obliterated_*", "obliterated", "bench_*", "reverseorbitllm_tourney/r*"):
...
def _build_model_choices() -> dict[str, str]:
    ...
    from reverseorbitllm.presets import list_all_presets
...
# Set ReverseOrbitLLM_HUB_TOKEN as a Space secret with write access to the org.
_HUB_COMMUNITY_ORG = os.environ.get("ReverseOrbitLLM_HUB_ORG", "ReverseOrbitLLM")
_HUB_COMMUNITY_TOKEN = os.environ.get("ReverseOrbitLLM_HUB_TOKEN")

# Import preset configs for Advanced Settings defaults
from reverseorbitllm.abliterate import METHODS as _PRESET_CONFIGS  # noqa: E402
from reverseorbitllm.prompts import (  # noqa: E402
    DATASET_SOURCES,
    get_source_choices,
...
def _validate_hub_repo(hub_repo: str) -> str:
    ...
    if (
        ...
        warnings.append(
            ...
            "Set HF_PUSH_TOKEN, HF_TOKEN, or ReverseOrbitLLM_HUB_TOKEN."
        )
    if warnings:
...
[ReverseOrbitLLM](https://github.com/elder-plinius/ReverseOrbitLLM).
...
## About ReverseOrbitLLM

ReverseOrbitLLM is an open-source tool for removing refusal behavior from language
models via activation engineering (abliteration). Learn more at
[github.com/elder-plinius/ReverseOrbitLLM](https://github.com/elder-plinius/ReverseOrbitLLM).
"""
...
def push_session_to_hub(
    ...
    if not token:
        yield (
            ...
            "or set `HF_PUSH_TOKEN`, `HF_TOKEN`, or `ReverseOrbitLLM_HUB_TOKEN` as an environment variable.",
            "",
        )
        return
    ...
    if refine_enabled and refine_passes > 0:
        ...
        try:
            ...
            from reverseorbitllm.prompts import load_dataset_source
    ...
    try:
        ...
        api.upload_folder(
            ...
            repo_id=repo_id,
            commit_message=f"ReverseOrbitLLM: {method} on {model_id}",
        )
    except Exception as e:
...
def _should_quantize(model_id: str, is_preset: bool = False) -> str | None:
    ...
    try:
        ...
        from reverseorbitllm.models.loader import (
            _available_gpu_memory_gb,
            _estimate_model_memory_gb,
        )
...
def _cleanup_disk():
    ...
    for p in Path("/tmp").glob("reverseorbitllm_offload_*"):
    ...
    for pattern in [
        "reverseorbitllm_chart_*.png",
        "reverseorbitllm_sweep_*.png",
        "reverseorbitllm_bench_*.png",
        "reverseorbitllm_bench_*.csv",
        "reverseorbitllm_export_*.zip",
    ]:
        for p in Path("/tmp").glob(pattern):
...
def _save_bench_csv(results: list[dict]) -> str | None:
    ...
    fd, path = tempfile.mkstemp(suffix=".csv", prefix="reverseorbitllm_bench_")
...
def _generate_analysis_figs(pipeline, model_label: str = "") -> list:
    ...
    try:
        ...
        from reverseorbitllm.analysis.visualization import (
            plot_angular_drift,
    ...
    if directions and strong_layers:
        try:
            ...
            from reverseorbitllm.analysis.visualization import plot_refusal_topology
...
def _figs_to_gallery(figs: list) -> list[tuple[str, str]]:
    ...
    for i, fig in enumerate(figs):
        try:
            fd, path = tempfile.mkstemp(suffix=".png", prefix=f"reverseorbitllm_chart_{i}_")
            os.close(fd)
...
def benchmark(
    ...
    for mi, method_key in enumerate(methods_to_test):
        ...
        try:
            from reverseorbitllm.telemetry import log_benchmark_from_dict
    ...
    from reverseorbitllm.evaluation.benchmark_plots import generate_benchmark_dashboard
...
def obliterate(
    ...
    if method == "adaptive":
        try:
            ...
            from reverseorbitllm.architecture_profiles import (
                detect_architecture,
    ...
    from reverseorbitllm.presets import is_gated
    ...
    try:
        ...
        try:
            from reverseorbitllm.telemetry import (
                log_benchmark_from_dict,
        ...
        if can_generate:
            ...
            if not can_generate:
                ...
                try:
                    offload_dir = tempfile.mkdtemp(prefix="reverseorbitllm_offload_")
                    model_reloaded = _load_model_to_device(
                        save_dir,
...
def strength_sweep(
    ...
    try:
        ...
        if valid:
            ...
            fd, path = tempfile.mkstemp(suffix=".png", prefix="reverseorbitllm_sweep_")
            os.close(fd)
...
def run_tourney(model_choice, selected_methods, dataset, quantization):
    ...
    from reverseorbitllm.tourney import (
        TourneyRunner,
        _checkpoint_matches,
        _load_checkpoint,
        render_bracket_html,
    ...
    tourney_dir = Path("/tmp/reverseorbitllm_tourney")
...
def export_artifacts():
    ...
    export_dir = tempfile.mkdtemp(prefix="reverseorbitllm_export_")
    ...
    with open(log_path, "w") as f:
        f.write("ReverseOrbitLLM Pipeline Log\n")
    ...
    fd, zip_path = tempfile.mkstemp(
        suffix=".zip", prefix=f"reverseorbitllm_{model_name.replace(' ', '_')}_{method}_"
    )
    os.close(fd)
...
with gr.Blocks(theme=THEME, css=CSS, js=_JS, title="ReverseOrbitLLM", fill_height=True) as demo:
    gr.HTML("""
    ...
    with gr.Tabs():
        ...
        with gr.Tab("Obliterate", id="obliterate"):
            ...
            gr.Markdown(
                ...
                "Opt out: set `ReverseOrbitLLM_TELEMETRY=0`.*",
...
client = Client("your-username/reverseorbitllm")
...
""")
            ...
            from reverseorbitllm.tourney import TOURNEY_METHODS as _ALL_TOURNEY_METHODS
...
All benchmark results from **every ReverseOrbitLLM Space** (including duplicated copies) are
...
To opt out, set the environment variable `ReverseOrbitLLM_TELEMETRY=0` before launching.*
""")
            ...
            def _load_leaderboard():
                ...
                try:
                    ...
                    if not is_telemetry_enabled():
                        return (
                            "Telemetry is disabled. Remove `ReverseOrbitLLM_TELEMETRY=0` or set it to `1` to re-enable.",
                            "",
                        )
                    ...
                    from reverseorbitllm.telemetry import _TELEMETRY_REPO
...
in weight space, not a deep behavioral change. ReverseOrbitLLM removes it in minutes.
...
- [GitHub](https://github.com/elder-plinius/ReverseOrbitLLM)
- [Paper](https://github.com/elder-plinius/ReverseOrbitLLM/tree/main/paper)
""")
...
def launch(
    ...
    Called by ``python app.py`` (HF Spaces) or ``reverseorbitllm ui`` (local).
    """
    demo.launch(
...
if __name__ == "__main__":
    ...
    _parser = _ap.ArgumentParser(description="ReverseOrbitLLM — Gradio UI")
