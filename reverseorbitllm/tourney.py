"""ReverseOrbitLLM Tourney — March Madness-style tournament to find the best abliteration method.
...
Usage (CLI):
    reverseorbitllm tourney meta-llama/Llama-3.1-8B-Instruct --hub-org my-org
...
def render_bracket(result: TourneyResult) -> str:
    ...
    lines = []
    lines.append(f"# ReverseOrbitLLM TOURNEY — {result.model}")
    lines.append("")
...
automated [ReverseOrbitLLM](https://github.com/elder-plinius/ReverseOrbitLLM) tournament
...
*Generated by [ReverseOrbitLLM](https://github.com/elder-plinius/ReverseOrbitLLM) tourney on {result.timestamp}*
"""
...
class TourneyRunner:
    ...
    def __init__(
        ...
        output_dir: str = "/tmp/reverseorbitllm_tourney",
    ...
    def run(self) -> TourneyResult:
        ...
        self.log(f"ReverseOrbitLLM TOURNEY")
    ...
    def run_iter(self, gpu_wrapper=None):
        ...
        if self.resume:
            ...
            if checkpoint and _checkpoint_matches(
                ...
                self.log("ReverseOrbitLLM TOURNEY — RESUMING")
        ...
        if not resuming:
            ...
            self.log("ReverseOrbitLLM TOURNEY")
            self.log(f"Model: {self.model_name}")
            self.log(f"Contenders: {n_methods} methods")
            self.log(f"Dataset: {self.dataset_key}")
    ...
    def _push_winner(self, result: TourneyResult):
        ...
        try:
            ...
            api.upload_folder(
                ...
                repo_id=repo_id,
                commit_message=(
                    f"ReverseOrbitLLM tourney: {winner.method} wins "
