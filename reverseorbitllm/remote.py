"""Remote execution support for ReverseOrbitLLM.

Run abliteration pipelines on remote GPU nodes via SSH. The remote machine
must have CUDA-capable GPUs and a Python environment. ReverseOrbitLLM will be
...
class RemoteConfig:
    ...
    remote_dir: str = "/tmp/reverseorbitllm_run"
...
class RemoteRunner:
    """Execute ReverseOrbitLLM commands on a remote machine via SSH."""
    ...
    def ensure_reverseorbitllm(self) -> bool:
        """Install or update reverseorbitllm on the remote if needed."""
        # Check if already installed
        check = self.run_ssh(
            f"{self.config.python} -c \"import reverseorbitllm; print(reverseorbitllm.__version__)\"",
            timeout=30,
        )
        if isinstance(check, subprocess.CompletedProcess) and check.returncode == 0:
            version = check.stdout.strip()
            self.on_log(f"ReverseOrbitLLM {version} already installed on remote")
            return True

        # Install from PyPI or git
        self.on_log("Installing reverseorbitllm on remote...")
        install_cmd = (
            f"{self.config.python} -m pip install --quiet "
            f"git+https://github.com/StellaAthena/ReverseOrbitLLM.git"
        )
        rc = self.run_ssh(install_cmd, stream=True, timeout=self.config.install_timeout)
        if rc != 0:
            self.on_log("[red]Failed to install reverseorbitllm on remote[/]")
            return False

        self.on_log("ReverseOrbitLLM installed successfully")
        return True
    ...
    def build_obliterate_command(
        ...
        """Build the remote reverseorbitllm CLI command."""
        remote_output = output_dir or f"{self.config.remote_dir}/output/{model.replace('/', '_')}"

        parts = [
            self._env_prefix() + self.config.python, "-m", "reverseorbitllm",
    ...
    def build_run_command(self, remote_config_path: str, output_dir: str | None = None, preset: str | None = None) -> str:
        """Build remote 'reverseorbitllm run' command."""
        parts = [
            self._env_prefix() + self.config.python, "-m", "reverseorbitllm",
    ...
    def build_tourney_command(
        ...
        """Build remote 'reverseorbitllm tourney' command."""
    ...
    def run_obliterate(
        ...
        **kwargs,
        ...
        # 3. Ensure reverseorbitllm is installed
        if not self.ensure_reverseorbitllm():
            return None
    ...
    def run_tourney(
        ...
        **kwargs,
        ...
        if self.config.sync_results:
            local_output = local_output_dir or f"/tmp/reverseorbitllm_tourney/{model.replace('/', '_')}"
            if self.sync_results_back(remote_output, local_output):
                return local_output
            return None
