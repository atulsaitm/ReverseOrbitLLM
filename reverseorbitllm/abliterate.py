...
Novel contributions (ReverseOrbitLLM):
...
from reverseorbitllm import device as dev  # noqa: E402 — must import before CUDA setup
...
from reverseorbitllm.models.loader import ModelHandle, load_model  # noqa: E402
from reverseorbitllm.strategies.utils import (  # noqa: E402
...
METHODS = {
    ...
    # ReverseOrbitLLM infrastructure. Each faithfully matches the original
    ...
    "gabliteration": {
        ...
        "description": (
            ...
            "projection (alpha=0.3, equivalent to ReverseOrbitLLM reg=0.231), "
        ...
        # which is equivalent to ReverseOrbitLLM reg = 1 - 0.769 = 0.231
    ...
    "heretic": {
        ...
        # ReverseOrbitLLM norm_preserve=False matches Heretic's default behavior.
        "norm_preserve": False,
        "regularization": 0.0,
        "refinement_passes": 1,
        "project_biases": False,
        ...
        # not ReverseOrbitLLM's norm-based layer_adaptive_strength.
        "layer_adaptive_strength": False,
        "safety_neuron_masking": False,
        "per_expert_directions": False,
        "attention_head_surgery": False,
        ...
        # directions (not SVD components). ReverseOrbitLLM float_layer_interpolation
...
from reverseorbitllm.prompts import BUILTIN_HARMFUL, BUILTIN_HARMLESS  # noqa: E402
...
class AbliterationPipeline:
    ...
    def _distill(self):
        ...
        For use_whitened_svd=True: covariance-normalized SVD (ReverseOrbitLLM novel)
        ...
        if self.direction_method == "leace":
            from reverseorbitllm.analysis.leace import LEACEExtractor
        ...
        #   knee_cosmic: ReverseOrbitLLM default (knee detection + COSMIC fusion)
        #   knee:        knee detection only (simplified ReverseOrbitLLM)
        ...
        if selection_method == "all_except_first":
            ...
            # Default: knee + COSMIC fusion (ReverseOrbitLLM standard)
    ...
    def _excise_inner(self, layers, arch, config, n_heads, t0):
        ...
        if bayesian_trials > 0 and self._strong_layers and self.handle:
            ...
            from reverseorbitllm.bayesian_optimizer import run_bayesian_optimization
        ...
        if self.use_lora_ablation and self._strong_layers:
            ...
            from reverseorbitllm.lora_ablation import (
                apply_lora_adapters,
    ...
    def _kl_optimize_corrections(self, layers: nn.ModuleList, total_modified: int):
        ...
        refusal + KL) with ReverseOrbitLLM's analysis-based pipeline:
        - Heretic: search-based (200 Optuna trials, expensive)
        - ReverseOrbitLLM: project first, measure KL, correct surgically (cheap)
    ...
    def _verify(self):
        ...
        if not generation_failed:
            ...
            from reverseorbitllm.evaluation.advanced_metrics import _is_refusal_detailed
            ...
            if harmful_responses:
                from reverseorbitllm.evaluation.advanced_metrics import (
        ...
        if (
            ...
            try:
                from reverseorbitllm.analysis.spectral_certification import SpectralCertifier
                ...
                if cert_results:
                    ...
                    from reverseorbitllm.analysis.spectral_certification import CertificationLevel
    ...
    def _build_metadata(self) -> dict:
        ...
        return {
            ...
            "references": [
                ...
                "ReverseOrbitLLM: Whitened SVD, EGA, CoT-aware, KL co-optimization, float interpolation (novel)",
            ],
            "strong_layers": self._strong_layers,
    ...
    def _rebirth(self) -> Path:
        ...
        if self._lora_adapters:
            from reverseorbitllm.lora_ablation import save_lora_adapters
        ...
        if self.push_to_hub:
            ...
            api.upload_folder(
                ...
                repo_id=repo_id,
                commit_message=f"ReverseOrbitLLM: abliterated {self.model_name} ({self.method})",
            )
