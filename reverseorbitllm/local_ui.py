"""Local UI launcher for ReverseOrbitLLM — beautiful terminal experience + Gradio web UI.

Usage:
    reverseorbitllm ui                          # launch with defaults
    reverseorbitllm ui --port 8080 --share      # public link on custom port
    reverseorbitllm ui --no-browser             # don't auto-open browser
    reverseorbitllm ui --auth user:pass         # basic auth
"""
...
def _print_launch_info(
    ...
    console.print(
        Panel(
            "\n".join(parts),
            title="[bold green]Launching ReverseOrbitLLM UI[/bold green]",
            border_style="green",
        )
    )
...
def launch_local_ui(
    ...
    """Launch the ReverseOrbitLLM Gradio UI with a beautiful local experience.

    This is the main entry point for ``reverseorbitllm ui``.
    """
    # ── Beautiful startup ──────────────────────────────────────────────
    if not quiet:
    ...
    console.print("[dim]Loading ReverseOrbitLLM UI (this may take a moment on first run)...[/dim]")
    start = time.time()
