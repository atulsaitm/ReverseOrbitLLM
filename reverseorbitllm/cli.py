"""CLI entry point for ReverseOrbitLLM — Master Ablation Suite."""
...
def _add_remote_args(parser):
    ...
    remote_group.add_argument(
        "--remote-dir", type=str, default="/tmp/reverseorbitllm_run",
        help="Working directory on the remote machine (default: /tmp/reverseorbitllm_run)",
    )
    remote_group.add_argument(
...
def main(argv: list[str] | None = None):
    ...
    tourney_parser.add_argument("--output-dir", type=str, default="/tmp/reverseorbitllm_tourney")
    tourney_parser.add_argument(
...
def _cmd_ui(args):
    from reverseorbitllm.local_ui import launch_local_ui
...
def _cmd_interactive():
    from reverseorbitllm.interactive import run_interactive
    run_interactive()
...
def _cmd_models(args):
    ...
    from reverseorbitllm.presets import get_presets_by_tier, list_all_presets
...
def _cmd_presets():
    ...
    from reverseorbitllm.study_presets import list_study_presets
    ...
    console.print(table)
    console.print(
        "\n[dim]Usage:  reverseorbitllm run config.yaml --preset quick\n"
        "   or:  set preset: quick in your YAML file[/dim]"
    )


def _cmd_run(args):
    from reverseorbitllm.config import StudyConfig
    from reverseorbitllm.runner import run_study
    ...
    if config.remote is not None:
        from reverseorbitllm.remote import RemoteConfig as _RC, RemoteRunner
...
def _cmd_info(args):
    from reverseorbitllm.models.loader import load_model
...
def _cmd_strategies():
    from reverseorbitllm.strategies import STRATEGY_REGISTRY
...
def _cmd_report(args):
    from reverseorbitllm.reporting.report import AblationReport, AblationResult
...
def _cmd_aggregate(args):
    from reverseorbitllm.community import aggregate_results, load_contributions
...
def _cmd_recommend(args):
    ...
    from reverseorbitllm.architecture_profiles import detect_architecture, enhance_profile_with_telemetry
    from reverseorbitllm.adaptive_defaults import format_recommendation, get_global_insights
...
def _cmd_tourney(args):
    from reverseorbitllm.tourney import TourneyRunner, render_bracket
...
def _cmd_abliterate(args):
    ...
    from reverseorbitllm.abliterate import METHODS, STAGES, AbliterationPipeline
    ...
    def make_display():
        ...
        header = Text.from_markup(
            f"[bold green]ReverseOrbitLLM — ABLITERATION PIPELINE[/]\n"
    ...
    try:
        from reverseorbitllm.telemetry import maybe_send_pipeline_report
...
def _cmd_gpu_calc(args):
    ...
    console.print(
        ...
        "and model architecture. See 'reverseorbitllm obliterate --help' for runtime options.[/]\n"
    )
...
def _make_remote_runner(args):
    ...
    from reverseorbitllm.remote import RemoteConfig, RemoteRunner
