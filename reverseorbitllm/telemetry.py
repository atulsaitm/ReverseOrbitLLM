...
by setting ReverseOrbitLLM_TELEMETRY=1 or calling enable_telemetry().  On
...
Architecture:
    1. Every benchmark/obliteration run appends a record to a local JSONL
       file (default: ~/.reverseorbitllm/telemetry.jsonl or /tmp/reverseorbitllm_telemetry.jsonl
       in containers).
    2. On HuggingFace Spaces, records are automatically synced to a central
       HuggingFace Dataset repo (default: pliny-the-prompter/ReverseOrbitLLM-TELEMETRY,
       configurable via ReverseOrbitLLM_TELEMETRY_REPO).  Each Space instance
...
_DEFAULT_TELEMETRY_REPO = "pliny-the-prompter/ReverseOrbitLLM-TELEMETRY"
_TELEMETRY_REPO = os.environ.get(
    "ReverseOrbitLLM_TELEMETRY_REPO",
...
def _telemetry_dir() -> Path:
    ...
    Priority order:
    1. ``ReverseOrbitLLM_DATA_DIR`` env var (explicit override)
    2. HuggingFace Spaces persistent storage (``/data/reverseorbitllm``)
       — survives container rebuilds and factory resets
    3. ``~/.reverseorbitllm`` (local installs)
    4. ``/tmp/reverseorbitllm_telemetry`` (last resort — does NOT survive rebuilds)
    ...
    explicit = os.environ.get("ReverseOrbitLLM_DATA_DIR")
    if explicit:
        p = Path(explicit)
        if _test_writable(p):
            logger.info("Telemetry storage: %s (ReverseOrbitLLM_DATA_DIR)", p)
            return p
        logger.warning(
            "ReverseOrbitLLM_DATA_DIR=%s is not writable, falling through", explicit
        )

    # 2. HF Spaces persistent storage at /data
    if _ON_HF_SPACES:
        data_root = Path("/data")
        hf_dir = data_root / "reverseorbitllm"
        ...
        if data_root.exists():
            ...
            logger.warning(
                "/data exists (mount_point=%s) but /data/reverseorbitllm is NOT writable. "
                "Persistent storage may not be enabled for this Space. "
                "Data will NOT survive factory rebuilds! "
                "Enable persistent storage in Space settings or set ReverseOrbitLLM_DATA_DIR.",
                is_mount,
            )

    # 3. Home directory (local installs)
    home_dir = Path.home() / ".reverseorbitllm"
    ...
    fallback = Path("/tmp/reverseorbitllm_telemetry")
...
def storage_diagnostic() -> dict[str, Any]:
    ...
    return {
        ...
        "explicit_data_dir": os.environ.get("ReverseOrbitLLM_DATA_DIR", ""),
...
def is_enabled() -> bool:
    """Check if telemetry is enabled (off by default, opt in with ReverseOrbitLLM_TELEMETRY=1).
    ...
    env = os.environ.get("ReverseOrbitLLM_TELEMETRY", default)
...
def _fetch_via_git_clone(repo: str, max_records: int) -> list[dict[str, Any]]:
    ...
    clone_dir = Path(tempfile.mkdtemp(prefix="reverseorbitllm_telemetry_"))
...
def push_to_hub(repo_id: str | None = None) -> bool:
    ...
    if not repo:
        logger.warning("No telemetry repo configured — set ReverseOrbitLLM_TELEMETRY_REPO")
        return False
