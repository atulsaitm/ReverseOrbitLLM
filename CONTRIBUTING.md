# Contributing to ReverseOrbitLLM
...
```bash
git clone https://github.com/elder-plinius/ReverseOrbitLLM.git
cd ReverseOrbitLLM
pip install -e ".[dev]"
```
...
```bash
ruff check reverseorbitllm/    # lint
ruff format reverseorbitllm/   # format
```
...
```bash
reverseorbitllm obliterate <model> --method advanced --contribute \
...
reverseorbitllm aggregate --format latex --min-runs 3
```
