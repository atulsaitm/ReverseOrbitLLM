#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# ReverseOrbitLLM Remote Benchmark Runner
#
...
# ── Defaults ─────────────────────────────────────────────────────────────────
SSH_KEY="${ReverseOrbitLLM_SSH_KEY:-$HOME/.ssh/hf_reverseorbitllm}"
SSH_HOST="${ReverseOrbitLLM_SSH_HOST:-}"
MODEL="${ReverseOrbitLLM_MODEL:-Qwen/Qwen2.5-0.5B-Instruct}"
MODELS=""
METHODS="${ReverseOrbitLLM_METHODS:-basic advanced aggressive surgical inverted nuclear}"
PROMPTS="${ReverseOrbitLLM_PROMPTS:-33}"
DRY_RUN=false
VERBOSE=false
...
if [[ -z "$SSH_HOST" ]]; then
  ...
  echo "  1. export ReverseOrbitLLM_SSH_HOST=your-username-spacename@ssh.hf.space"
...
if [[ ! -f "$SSH_KEY" ]]; then
  ...
  echo ""
  echo "Either:"
  echo "  1. Place your HF Space SSH key at ~/.ssh/hf_reverseorbitllm"
  echo "  2. Set ReverseOrbitLLM_SSH_KEY=/path/to/key"
  echo "  3. Pass --key /path/to/key"
  exit 1
fi

echo "╔══════════════════════════════════════════════════════════════╗"
echo "║          ReverseOrbitLLM — Remote GPU Benchmark                 ║"
echo "╠══════════════════════════════════════════════════════════════╣"
echo "║  Host:     $SSH_HOST"
echo "║  Models:   $MODELS"
...
import reverseorbitllm.abliterate as _abl
...
from reverseorbitllm.abliterate import AbliterationPipeline, METHODS, HARMFUL_PROMPTS, HARMLESS_PROMPTS
...
print(f"\n{'='*60}")
print(f"ReverseOrbitLLM BENCHMARK")
print(f"{'='*60}")
...
for model_name in MODELS_LIST:
    ...
    for method in METHODS_LIST:
        ...
        outdir = f"/tmp/reverseorbitllm_bench_{method}"
        t0 = time.time()
        pipeline = None
...
if ! ssh "${SSH_OPTS[@]}" "$SSH_HOST" "echo 'SSH_OK'" 2>/tmp/reverseorbitllm_ssh_debug.log; then
  echo ""
  echo "ERROR: SSH connection failed!"
  echo ""
  echo "Debug output:"
  cat /tmp/reverseorbitllm_ssh_debug.log
  echo ""
  ...
  echo ""
  rm -f /tmp/reverseorbitllm_ssh_debug.log
  exit 1
fi
rm -f /tmp/reverseorbitllm_ssh_debug.log
...
REMOTE_SCRIPT_FILE=$(mktemp /tmp/reverseorbitllm_bench_XXXXXX.py)
