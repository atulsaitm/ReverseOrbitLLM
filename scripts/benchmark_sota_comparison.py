#!/usr/bin/env python3
"""ReverseOrbitLLM vs SOTA — Head-to-Head Benchmark Comparison.

Runs faithful reproductions of competing abliteration methods against
ReverseOrbitLLM variants on any specified model, producing publication-ready
...
ReverseOrbitLLM variants:
  5. ReverseOrbitLLM surgical — Full SOTA MoE-aware pipeline
  6. ReverseOrbitLLM informed — Analysis-guided auto-configuration
  7. ReverseOrbitLLM optimized — Bayesian + whitened SVD + SAE (max ReverseOrbitLLM)
...
from reverseorbitllm.abliterate import (  # noqa: E402
    AbliterationPipeline,
    METHODS,
    HARMFUL_PROMPTS,
    HARMLESS_PROMPTS,
)
from reverseorbitllm.evaluation.benchmarks import BenchmarkRunner  # noqa: E402
...
# ReverseOrbitLLM variants
ReverseOrbitLLM_METHODS = ["surgical", "informed", "optimized"]

# Default comparison set
DEFAULT_METHODS = BASELINE_METHODS + ReverseOrbitLLM_METHODS
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="ReverseOrbitLLM vs SOTA — Head-to-Head Benchmark",
    ...
    parser.add_argument(
        "--output-dir", default="/tmp/reverseorbitllm_benchmark",
        help="Directory for abliterated model outputs (default: /tmp/reverseorbitllm_benchmark)",
    )
    parser.add_argument(
...
def measure_refusal_rate(
    ...
    from reverseorbitllm.evaluation.advanced_metrics import refusal_rate
...
def run_single_method(
    ...
    try:
        ...
        if args.full_eval:
            ...
            try:
                from reverseorbitllm.evaluation.heretic_eval import run_full_heretic_eval
...
def format_comparison_table(results: list[MethodResult]) -> str:
    ...
    lines.append("")
    lines.append("=" * 115)
    lines.append("ReverseOrbitLLM vs SOTA — Head-to-Head Benchmark Comparison")
    lines.append("=" * 115)
    lines.append("")

    # Separator between baselines and ReverseOrbitLLM
    ...
    reverseorbitllm_results = [r for r in results if r.method not in BASELINE_METHODS]
    ...
    if reverseorbitllm_results:
        lines.append("  ReverseOrbitLLM:")
        for r in reverseorbitllm_results:
            if r.error:
                lines.append(f"  {r.label:<33} {'FAILED':>10}  {r.error[:60]}")
            else:
                lines.append(
...
def main():
    ...
    print("=" * 70)
    print("  ReverseOrbitLLM vs SOTA — Head-to-Head Benchmark")
