...
# ReverseOrbitLLM — Master Ablation Suite
...
```bash
git clone https://github.com/elder-plinius/ReverseOrbitLLM
cd ReverseOrbitLLM
...
The `reverseorbitllm ui` command auto-detects your GPU, prints hardware-specific model recommendations, and opens the browser automatically. Supports `--share` for public links, `--port` for custom ports, and `--auth user:pass` for access control.
