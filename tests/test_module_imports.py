...
class TestTopLevelImports:
    """Verify reverseorbitllm top-level exports."""

    def test_set_seed(self):
        from reverseorbitllm import set_seed
        assert callable(set_seed)

    def test_run_sweep(self):
        from reverseorbitllm import run_sweep
        assert callable(run_sweep)

    def test_sweep_config(self):
        from reverseorbitllm import SweepConfig
        cfg = SweepConfig(
            model_name="test",
    ...
    def test_sweep_result(self):
        from reverseorbitllm import SweepResult
        r = SweepResult(
...
class TestEvaluationImports:
    ...
    def test_refusal_rate_with_ci(self):
        from reverseorbitllm.evaluation import refusal_rate_with_ci
        result = refusal_rate_with_ci(["Sure, here you go."], mode="combined")
        assert result["rate"] == 0.0
        assert result["n_samples"] == 1

    def test_random_direction_ablation(self):
        from reverseorbitllm.evaluation import random_direction_ablation
        assert callable(random_direction_ablation)

    def test_direction_specificity_test(self):
        from reverseorbitllm.evaluation import direction_specificity_test
        assert callable(direction_specificity_test)

    def test_run_benchmarks(self):
        from reverseorbitllm.evaluation import run_benchmarks
        assert callable(run_benchmarks)

    def test_compare_models(self):
        from reverseorbitllm.evaluation import compare_models
...
class TestDirectImports:
    ...
    def test_baselines(self):
        from reverseorbitllm.evaluation.baselines import (
            BaselineResult,
        )
        assert BaselineResult is not None

    def test_lm_eval_integration(self):
        from reverseorbitllm.evaluation.lm_eval_integration import (
            run_benchmarks,
        )
        assert callable(run_benchmarks)
