...
def _reset_telemetry():
    import reverseorbitllm.telemetry as t
    t._enabled = None
...
class TestTelemetryConfig:
    ...
    def test_disable_via_env_zero(self):
        with patch.dict(os.environ, {"ReverseOrbitLLM_TELEMETRY": "0"}):
            _reset_telemetry()
            assert not is_enabled()

    def test_disable_via_env_false(self):
        with patch.dict(os.environ, {"ReverseOrbitLLM_TELEMETRY": "false"}):
            _reset_telemetry()
            assert not is_enabled()

    def test_enable_via_env_explicit(self):
        with patch.dict(os.environ, {"ReverseOrbitLLM_TELEMETRY": "1"}):
            _reset_telemetry()
            assert is_enabled()
...
class TestSendReport:
    ...
    def test_does_not_send_when_disabled(self):
        disable_telemetry()
        with patch("reverseorbitllm.telemetry._send_sync") as mock_send:
    ...
    def test_send_failure_is_silent(self):
        enable_telemetry()
        with patch("reverseorbitllm.telemetry._send_sync", side_effect=Exception("network down")) as mock_send:
...
class TestPipelineIntegration:
    ...
    def test_does_nothing_when_disabled(self):
        disable_telemetry()
        with patch("reverseorbitllm.telemetry.send_report") as mock_send:
...
class TestStorageHelpers:
    ...
    def test_test_writable_unwritable(self):
        ...
        assert not _test_writable(Path("/proc/reverseorbitllm_test"))
...
class TestHubRestore:
    ...
    def test_restore_skips_when_no_repo(self):
        with patch("reverseorbitllm.telemetry._TELEMETRY_REPO", ""):
    ...
    def test_restore_deduplicates(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            try:
                ...
                with patch("reverseorbitllm.telemetry.fetch_hub_records", return_value=hub_records):
    ...
    def test_restore_only_runs_once(self):
        ...
        with patch("reverseorbitllm.telemetry._TELEMETRY_REPO", "test/repo"):
            with patch("reverseorbitllm.telemetry.fetch_hub_records", return_value=[]):
                restore_from_hub()
