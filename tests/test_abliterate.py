...
from reverseorbitllm.abliterate import (
    HARMFUL_PROMPTS,
    HARMLESS_PROMPTS,
    METHODS,
    STAGES,
...
from reverseorbitllm.models.loader import ModelHandle
...
class TestProjectOutAdvanced:
    def test_norm_preserving(self):
        ...
        without_preserve_norm_sq = (
            original_norm**2 - (module.o_proj.weight.data @ direction).pow(2).sum().item()
        )
...
class TestAttentionFullProjection:
    ...
    def test_qkv_all_projected(self):
        ...
        from reverseorbitllm.abliterate import _ATTN_IN_NAMES, _ATTN_OUT_NAMES
    ...
    def test_project_all_does_not_early_return(self):
        ...
        from reverseorbitllm.abliterate import _FFN_IN_NAMES
...
class TestSAEAbliteration:
    def test_sae_train_and_reconstruct(self):
        ...
        from reverseorbitllm.analysis.sae_abliteration import train_sae
        ...
        # Forward pass should work — move x to the same device as the SAE
...
class TestInvertedMethod:
    ...
    def test_moe_expert_safety_classification(self):
        ...
        import reverseorbitllm.abliterate as abl_module
...
class TestActivationCollection:
    def test_collect_activations(self, handle):
        ...
        from reverseorbitllm.strategies.utils import get_layer_modules
        ...
        def _batch_tokenizer(prompt, **kwargs):
            batch_size = len(prompt) if isinstance(prompt, list) else 1
            return {
                "input_ids": torch.randint(0, 1000, (batch_size, 5)),
                "attention_mask": torch.ones(batch_size, 5, dtype=torch.long),
        ...
        handle.tokenizer.side_effect = _batch_tokenizer
        handle.tokenizer.return_value = None
...
class TestRouterProfilingHooks:
    ...
    def test_hooks_installed(self):
        ...
        try:
            ...
            for h in hooks:
                h.remove()
            setattr(abl_module, "get_ffn_module", orig_get_ffn)
