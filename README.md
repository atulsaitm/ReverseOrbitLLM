...
<p align="center">
  <a href="https://huggingface.co/spaces/pliny-the-prompter/reverseorbitllm">
    <img src="https://img.shields.io/badge/%F0%9F%A4%97%20Hugging%20Face-Spaces-blue" alt="Open in HF Spaces">
  </a>
  &nbsp;
  <a href="https://colab.research.google.com/github/elder-plinius/ReverseOrbitLLM/blob/main/notebooks/abliterate.ipynb">
    <img src="https://colab.research.google.com/assets/colab-badge.svg" alt="Open in Colab">
  </a>
</p>

<p align="center">
  <b><a href="https://huggingface.co/spaces/pliny-the-prompter/reverseorbitllm">Try it now on HuggingFace Spaces</a></b> — runs on ZeroGPU, free daily quota with HF Pro. No setup, no install, just obliterate.
</p>

---

**ReverseOrbitLLM** is the most advanced open-source toolkit for understanding and removing refusal behaviors from large language models — and every single run makes it smarter. It implements abliteration — a family of techniques that identify and surgically remove the internal representations responsible for content refusal, without retraining or fine-tuning. The result: a model that responds to all prompts without artificial gatekeeping, while preserving its core language capabilities.

But ReverseOrbitLLM is more than a tool — **it's a distributed research experiment.** Every time you obliterate a model with telemetry enabled, your run contributes anonymous benchmark data to a growing, crowd-sourced dataset that powers the next generation of abliteration research. Refusal directions across architectures. Hardware-specific performance profiles. Method comparisons at scale no single lab could achieve. **You're not just using a tool — you're co-authoring the science.**
...
Built on published research from [Arditi et al. (2024)](https://arxiv.org/abs/2406.11717), [Gabliteration (arXiv:2512.18901)](https://arxiv.org/abs/2512.18901), [grimjim's norm-preserving biprojection (2025)](https://huggingface.co/grimjim), [Turner et al. (2023)](https://arxiv.org/abs/2308.10248), and [Rimsky et al. (2024)](https://arxiv.org/abs/2312.06681), ReverseOrbitLLM implements precision liberation in a single command:

```bash
reverseorbitllm obliterate meta-llama/Llama-3.1-8B-Instruct --method advanced
```

Or zero commands — just [open the Colab notebook](https://colab.research.google.com/github/elder-plinius/ReverseOrbitLLM/blob/main/notebooks/abliterate.ipynb) and hit Run All.

## What it does

ReverseOrbitLLM does four things — and the community does the fifth (see [Community-powered research](#community-powered-research--every-run-advances-the-science) below):
...
## What makes ReverseOrbitLLM unique

Several capabilities distinguish ReverseOrbitLLM from existing public tools:
...
ReverseOrbitLLM implements several techniques that go beyond prior work:
...
# Or with options:
reverseorbitllm ui --port 8080          # custom port
reverseorbitllm ui --share              # generate a public share link
reverseorbitllm ui --no-browser         # don't auto-open browser
reverseorbitllm ui --auth user:pass     # add basic auth

# → opens http://localhost:7860 automatically
```

The `reverseorbitllm ui` command adds a Rich terminal startup with GPU detection and hardware-appropriate model recommendations. You can also run `python app.py` directly (same thing the Space uses).

### 3. Google Colab (free GPU)

[![Open in Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/elder-plinius/ReverseOrbitLLM/blob/main/notebooks/abliterate.ipynb)
...
reverseorbitllm interactive

# Direct obliteration — one command, one model, done
reverseorbitllm obliterate meta-llama/Llama-3.1-8B-Instruct --method advanced

# With all options
reverseorbitllm obliterate meta-llama/Llama-3.1-8B-Instruct \
    --method surgical \
    --output-dir ./liberated \
    --contribute --contribute-notes "A100 80GB, default prompts"

# Run a full ablation study from a YAML config
reverseorbitllm run examples/gpt2_layer_ablation.yaml

# Browse available models by compute tier
reverseorbitllm models
reverseorbitllm models --tier small      # filter by VRAM requirement

# Browse ablation presets
reverseorbitllm presets

# List available strategies
reverseorbitllm strategies

# Inspect model architecture before abliterating
reverseorbitllm info meta-llama/Llama-3.1-8B-Instruct

# Aggregate community results
reverseorbitllm aggregate --format summary
reverseorbitllm aggregate --format latex --metric refusal_rate --min-runs 3
```

### 5. Python API (full programmatic control)

For researchers who want to integrate ReverseOrbitLLM into their own pipelines:
...
```bash
reverseorbitllm run my_study.yaml
```

## Two intervention paradigms

ReverseOrbitLLM supports both permanent and reversible liberation:
...
```python
from reverseorbitllm.analysis import SteeringVectorFactory, SteeringHookManager
from reverseorbitllm.analysis.steering_vectors import SteeringConfig
...
The research core of ReverseOrbitLLM. Each module maps a different aspect of how the chains are forged — because precision liberation requires understanding the geometry before cutting:
...
```python
from reverseorbitllm.analysis import (
...
Beyond targeted liberation, ReverseOrbitLLM is a general-purpose ablation suite for mapping the internals of any transformer:
...
ReverseOrbitLLM ships with presets for 116 models organized by compute requirement:
...
ReverseOrbitLLM automatically shards models across multiple GPUs when they don't fit on a single card. It also supports remote execution over SSH, so you can run the pipeline on a GPU server from your laptop.

### How model sharding works

When you have multiple GPUs, ReverseOrbitLLM uses accelerate's `device_map="auto"` to split the model's layers across all available GPUs. This is **naive pipeline parallelism** — layers are distributed evenly, but only one GPU computes at a time as activations flow sequentially through the layer stack. The other GPUs hold their assigned layers in memory but are idle until their turn.
...
reverseorbitllm obliterate bigmodel/200B --gpus all

# Use only GPUs 0-3
reverseorbitllm obliterate bigmodel/200B --gpus 0,1,2,3

# Use a specific pair
reverseorbitllm obliterate meta-llama/Llama-3.1-70B-Instruct --gpus 2,5
```
...
```bash
# Default: bfloat16
reverseorbitllm obliterate meta-llama/Llama-3.1-70B-Instruct
...
reverseorbitllm obliterate meta-llama/Llama-3.1-405B-Instruct \
...
reverseorbitllm gpu-calc meta-llama/Llama-3.1-70B-Instruct --gpu-mem 24

# Manual: specify params and precision
reverseorbitllm gpu-calc --params 70 --dtype bfloat16 --gpu-mem 80

# MoE models: specify active params separately
reverseorbitllm gpu-calc --params 117 --active-params 13 --dtype bfloat16 --gpu-mem 80
```
...
reverseorbitllm obliterate EleutherAI/pythia-12b --data-parallel
```
...
Run the full pipeline on a remote GPU node from your local machine. ReverseOrbitLLM handles SSH connection, auto-installs itself on the remote if needed, streams logs in real time, and copies results back when done.
...
reverseorbitllm obliterate meta-llama/Llama-3.1-70B-Instruct \
    ...
    --ssh-key ~/.ssh/id_rsa \
    --ssh-port 2222 \
    --remote-dir /data/reverseorbitllm \
...
Remote execution also works with `reverseorbitllm run` (YAML configs) and `reverseorbitllm tourney` (method comparison). You can specify remote settings in YAML:
...
remote:
  ...
  user: root
  ssh_key: ~/.ssh/id_rsa
  remote_dir: /tmp/reverseorbitllm_run
...
3. Installs reverseorbitllm if not already present
4. Uploads config files if using `reverseorbitllm run`
...
| Model doesn't fit on 1 GPU | Use `--gpus` with the **minimum** number of GPUs that fits. Run `reverseorbitllm gpu-calc` to find that number. |
...
| Not sure how many GPUs you need | Run `reverseorbitllm gpu-calc <model> --gpu-mem <your_vram>` for an estimate. |
...
```bash
reverseorbitllm run examples/preset_quick.yaml
```

## How it compares

| Capability | ReverseOrbitLLM | TransformerLens | Heretic | FailSpy abliterator | RepEng | SAELens |
|---|---|---|---|---|---|---|
...
export ReverseOrbitLLM_TELEMETRY=1
```

**What gets collected:** model name, method, aggregate benchmark scores (refusal rate, perplexity, coherence, KL divergence), hardware info, and timestamps. **What never gets collected:** prompts, outputs, IP addresses, user identity, or anything that could trace back to you. The full schema is in `reverseorbitllm/telemetry.py` — read every line, we have nothing to hide.
...
```python
from reverseorbitllm import save_contribution, load_contributions, aggregate_results
from reverseorbitllm.abliterate import AbliterationPipeline
...
If you use ReverseOrbitLLM in your research, please cite:

```bibtex
@software{reverseorbitllm2026,
  title     = {ReverseOrbitLLM: An Open Platform for Analysis-Informed
               Refusal Removal in Large Language Models},
  author    = {{ReverseOrbitLLM Contributors}},
  year      = {2026},
  url       = {https://github.com/elder-plinius/ReverseOrbitLLM},
...
- **Open source** — [GNU Affero General Public License v3.0](LICENSE) (AGPL-3.0). You can freely use, modify, and distribute ReverseOrbitLLM under AGPL terms. If you run a modified version as a network service (SaaS), you must release your source code to users under the same license.

- **Commercial** — Organizations that cannot comply with AGPL obligations (e.g., proprietary SaaS, closed-source products, internal tools where source disclosure is not possible) can purchase a commercial license. Contact us via [GitHub Issues](https://github.com/elder-plinius/ReverseOrbitLLM/issues) for pricing and terms.
